...
def listen_for_interrupt():
    ...
    with mic as source:
        ...
        while not tts_should_stop.is_set():
            try:
                print("🎧 Awaiting possible interruption (1 sec window)...")
                audio = local_recognizer.listen(source, timeout=1)
                ...
                # Nothing heard in this interval — try again
                continue
            except Exception as e:
                print(f"⚠️ Error while listening for interrupt: {e}")
                continue
